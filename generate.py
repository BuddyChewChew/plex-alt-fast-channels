    ...
    for attempt in range(4):
        try:
            ...
            try:
                ...
                # Status was 200/201 but body wasn't valid JSON – log everything
                # needed to diagnose it (WAF/CDN block, empty body, HTML, etc.)
                logger.warning(
                    "Token attempt %d failed (%s): non-JSON body on HTTP %d. "
                    "content-type=%s len=%d server=%s cf-ray=%s body=%r",
                    attempt + 1, region, resp.status_code,
            ...
            logger.warning("Token attempt %d failed (%s): 200 response but no authToken. body=%r",
                            attempt + 1, region, resp.text[:300])
            time.sleep(5)
        except Exception as exc:
